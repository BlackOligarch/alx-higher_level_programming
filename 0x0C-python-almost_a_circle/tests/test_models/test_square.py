...
class TestSquare(unittest.TestCase):
    ...
    def test_set_size(self) -> None:
        ...
        # if the size is correct, then the width and height must be the same
        self.ass    with self.assertRaisesRegex(ValueError, "width must be > 0"):
            self.sq1.size = -5
            self.sq2.size = -19
...
class TestCreateOnSquare(unittest.TestCase):
    ...
    def test_create_update_lt = {"x": 10, "id": 12, "y": 0, "size": 5}
        self.assertEqual(sq1.to_dictionary(), expected_result)
...
class TestLoadFromFileOnSquare(unittest.TestCase):
    ...
    def test_file_exists(self) -> None:
        ...
        try:
            ...
            r2 = instances[0]
            self.assertEqual(r2.__str__(), "[Square] (12) 2/2 - 8")ertEqual(self.sq1.width, self.sq1.size)
        self.assertEqual(self.sq1.width, self.sq1.size)
